        ...
        def calculate_price():
            while True:
                try:
                    ...
                    current = self.web3.eth.getStorageAt(config.oracle_address.address, 3)
                    self.price = float(Wad(Web3.toInt(hexstr=f'0x{Web3.toHex(current)[34:].lstrip("0")}')))

                    next = self.web3.eth.getStorageAt(config.oracle_address.address, 4)
                    self.next_price = float(Wad(Web3.toInt(hexstr=f'0x{Web3.toHex(next)[34:].lstrip("0")}')))
